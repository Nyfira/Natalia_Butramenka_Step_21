...
// Задача 10
let x = prompt(`Загадайте число от 0 до 100.  Ваше число > , < или = 50?`);
let a = 0;
let b = 100;
let result = 0;
do {
    if (x == `=`) {
      result= ((a+b)%2 !== 0) ? (a+b+1)/2 : (a+b)/2;
      alert(`Ваше число ${result}`);
    } else if (x == `>`) { 
        a = ((a+b)%2 !== 0) ? (a+b+1)/2 : (a+b)/2;
        result= ((a+b)%2 !== 0) ? (a+b+1)/2 : (a+b)/2;
        x = prompt(`Ваше число > , < или = ${result}?`);
    } else if (x == `<`) {
        b = ((a+b)%2 !== 0) ? (a+b-1)/2 : (a+b)/2;
        result= ((a+b)%2 !== 0) ? (a+b+1)/2 : (a+b)/2;
        x = prompt(`Ваше число > , < или = ${result}?`);        
    } else if (x == `<` && a == 0 && b == 1 ) { 
        x = prompt(`Ваше число 0`);
    };
} while (true) {
